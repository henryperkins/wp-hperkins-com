...
import { isValidFileType } from "./utils.mjs";
import { useMaxWidthObserver } from "./use-max-width-observer.mjs";
import {
...
var isExternalImage = (id, url) => url && !id && !isBlobURL(url);
...
function ImageEdit({
  ...
  const [placeholderResizeListener, { width: placeholderWidth }] = useResizeObserver();
  ...
  function onSelectURL(newURL) {
    const normalizedNewURL = getProtocol(newURL) ? newURL : prependHTTPS(newURL);
  ...
  const mediaPreview = !!url && /* @__PURE__ */ jsx(
    "img",
    {
      alt: __("Edit image"),
      title: __("Edit image"),
  ...
  const { lockUrlControls = false, lockUrlControlsMessage } = useSelect(
    (select) => {
      ...
      return {
        ...
        lockUrlControlsMessage: blockBindingsSource?.label ? sprintf(
  ...
  const placeholder = (content) => {
    return /* @__PURE__ */ jsxs(
      Placeholder,
      {
        ...
        instructions: !lockUrlControls && !isSmallContainer && __(

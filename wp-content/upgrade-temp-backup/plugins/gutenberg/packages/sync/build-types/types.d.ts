 ...
 */
export type ProviderOn = <K extends keyof ProviderEventMap>(event: K, callback: (data: ProviderEventMap[K]) => void) => void;
 ...
 */
export type ConnectionErrorCode = 'authentication-error' | 'connection-expired' | 'connection-limit-exceeded' | 'unknown-error';
/**
...
export interface RecordHandlers {
    ...
    editRecord: (data: Partial<ObjectData>, options?: {
        undoIgnore?: boolean;
    }) => void;
...
export interface SyncConfig {
    ...
    createAwareness?: (ydoc: Y.Doc, objectId?: ObjectID) => Awareness | undefined;
...
export interface SyncManager {
    ...
    loadCollection: (syncConfig: SyncConfig, objectType: ObjectType, handlers: CollectionHandlers) => Promise<void>;
    undoManager: SyncUndoManager | undefined;
    unload: (objectType: ObjectType, objectId: ObjectID) => void;
    update: (objectType: ObjectType, objectId: ObjectID | null, changes: Partial<ObjectData>, origin: string, options?: SyncManagerUpdateOptions) => void;
}
export interface SyncUndoManager extends WPUndoManager<ObjectData> {
    addToScope: (ymap: Y.Map<any>, handlers: Pick<RecordHandlers, 'addUndoMeta' | 'restoreUndoMeta'>) => void;
    stopCapturing: () => void;
}

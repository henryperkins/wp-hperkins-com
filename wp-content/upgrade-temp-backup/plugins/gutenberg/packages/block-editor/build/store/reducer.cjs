...
__export(reducer_exports, {
  ...
  lastFocus: () => lastFocus,
  listViewContentPanelOpen: () => listViewContentPanelOpen,
  ...
  template: () => template,
  viewportModalClientIds: () => viewportModalClientIds,
...
function withDerivedBlockEditingModes(reducer) {
  return (state, action) => {
    ...
    switch (action.type) {
      ...
      case "UPDATE_SETTINGS": {
        if (state?.settings?.[import_private_keys.sectionRootClientIdKey] !== nextState?.settings?.[import_private_keys.sectionRootClientIdKey] || !!state?.settings?.disableContentOnlyForUnsyncedPatterns !== !!nextState?.settings?.disableContentOnlyForUnsyncedPatterns || !!state?.settings?.[import_private_keys.isIsolatedEditorKey] !== !!nextState?.settings?.[import_private_keys.isIsolatedEditorKey]) {
          return {
            ...nextState,

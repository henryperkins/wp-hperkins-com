...
import { PREFERENCES_DEFAULTS, SETTINGS_DEFAULTS } from "./defaults.mjs";
import { insertAt, moveTo } from "./array.mjs";
import { sectionRootClientIdKey, isIsolatedEditorKey } from "./private-keys.mjs";
...
function isUpdatingSameBlockAttribute(action, lastAction) {
  return action.type === "UPDATE_BLOCK_ATTRIBUTES" && lastAction !== void 0 && lastAction.type === "UPDATE_BLOCK_ATTRIBUTES" && fastDeepEqual(action.clientIds, lastAction.clientIds) && hasSameKeys(action.attributes, lastAction.attributes);
}
...
function getDerivedBlockEditingModesForTree(state, treeClientId = "") {
  ...
  traverseBlockTree(state, treeClientId, (block) => {
    ...
    if (hasEditedContentOnlySection) {
      isWithinEditedContentOnlySection = clientId === state.editedContentOnlySection || !!findParentInClientIdsList(state, clientId, [

...
var deleteEntityRecord = (kind, name, recordId, query, { __unstableFetch = import_api_fetch.default, throwOnError = false } = {}) => async ({ dispatch, resolveSelect }) => {
...
var editEntityRecord = (kind, name, recordId, edits, options = {}) => ({ select, dispatch }) => {
  ...
  if (entityConfig.syncConfig) {
    ...
    const origin = options.undoIgnore ? import_sync.LOCAL_UNDO_IGNORED_ORIGIN : import_sync.LOCAL_EDITOR_ORIGIN;
...
var clearEntityRecordEdits = (kind, name, recordId) => ({ select, dispatch }) => {
  const entityConfig = select.getEntityConfig(kind, name);
  (0, import_log_entity_deprecation.default)(kind, name, "clearEntityRecordEdits");
  if (!entityConfig) {
    throw new Error(
      `The entity being edited (${kind}, ${name}) does not have a loaded config.`
    );
      ...
      if (isAutosave) {
        ...
        if (entityConfig.syncConfig) {
          (0, import_sync.getSyncManager)()?.update(
            ...
            recordId,
            updatedRecord,
            import_sync.LOCAL_UNDO_IGNORED_ORIGIN,
            { isSave: true }
          );
        }
      }

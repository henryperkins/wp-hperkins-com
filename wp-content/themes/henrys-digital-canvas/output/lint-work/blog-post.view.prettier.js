(function (wp) {
  ...
  function normalizePost(post, index) {
    ...
    const readingTime = ensureString(
      ...
      utils.estimateReadingTimeLabel
        ? utils.estimateReadingTimeLabel(contentHtml || content || excerpt)
        : "1 min read",
    );
  ...
  function renderMarkdownBlock(content, keyPrefix, headingQueue) {
    ...
    while (i < lines.length) {
      ...
      if (line.startsWith("- ") || /^\d+\.\s/.test(line)) {
        ...
        while (
          j < lines.length &&
          (lines[j].startsWith("- ") || /^\d+\.\s/.test(lines[j]))
        ) {
          ...
          items.push(
            h(
              ...
              renderInline(text, keyPrefix + "-li-inline-" + String(j)),
            ),
          );
          j++;
        }
  ...
  function buildHighlightRules(lang) {
    ...
    var commonRules = [
      ...
      {
        type: "number",
        pattern: /\b(?:0[xXoObB][\da-fA-F_]+|\d+\.?\d*(?:[eE][+-]?\d+)?)\b/y,
      },
    ];
    ...
    if (lang === "css") {
      return [
        ...
        {
          type: "number",
          pattern: /\b\d+\.?\d*(?:%|px|em|rem|vh|vw|fr|s|ms|deg|ch|ex)?\b/y,
        },
    ...
    if (lang === "yaml") {
      return [
        ...
        { type: "keyword", pattern: /\b(?:true|false|null|yes|no|on|off)\b/y },
        {
          type: "number",
          pattern: /\b(?:0[xX][\da-fA-F_]+|\d+\.?\d*(?:[eE][+-]?\d+)?)\b/y,
        },
    ...
    if (lang === "bash") {
      return [
        ...
        {
          type: "keyword",
          pattern: new RegExp("\\b(?:" + bashKeywords + ")\\b", "y"),
        },
    ...
    container
      ...
      .forEach(function (preEl) {
        ...
        const language = getLanguageFromClassNames(
          codeEl.getAttribute("class") || preEl.getAttribute("class") || "",
        );
  ...
  function normalizeComment(comment, index) {
    ...
    return {
      ...
      date: date,
      parentId:
        typeof comment.parent === "number" && Number.isFinite(comment.parent)
          ? comment.parent
          : 0,
  ...
  function fetchCommentsPage(url, options) {
    ...
    return fetch(url, {
      ...
      if (!response.ok) {
        const error = new Error(
          "WordPress comments request failed with status " + response.status,
        );
        error.status = response.status;
        throw error;
      }
      ...
      const total = parseInt(
        response.headers.get("x-wp-total") || String(payload.length),
        10,
      );
    ...
    return fetchCommentsPage(firstPageUrl.toString(), options).then(
      function (firstPage) {
        ...
        for (let page = 2; page <= cappedTotalPages; page++) {
          ...
          pagePromises.push(fetchCommentsPage(pageUrl.toString(), options));
        }
  ...
  function submitComment(config, payload) {
    ...
    return fetch(config.commentSubmitEndpoint, {
      ...
      if (!response.ok) {
        throw new Error(
          body && body.error ? body.error : "Comment submission failed",
        );
      }

      return {
        comment:
          body && body.comment ? normalizeComment(body.comment, 0) : null,
        moderationStatus:
          body && body.moderationStatus === "approved" ? "approved" : "pending",
      };
    });
  }
    ...
    return h(
      ...
      h(
        ...
        props.description
          ? h(
              "p",
              { className: "hdc-blog-post__jump-nav-description" },
              props.description,
            )
          : null,
        h(
                ...
                h(
                  "a",
                  {
                    className:
                      ...
                      (isActive ? " hdc-blog-post__jump-nav-link--active" : ""),
                    href: item.href,
        ...
        h(
          ...
          h(
            ...
            comment.authorAvatarUrl
              ...
              : h(
                  "span",
                  { className: "hdc-blog-post__comment-avatar-fallback" },
  ...
  function BlogPostApp(props) {
    ...
    const [isCommentVerificationPending, setIsCommentVerificationPending] =
      useState(false);
          ...
          if (currentPost) {
            try {
              const postPayload = await fetchJson(
                config.endpointBase + encodeURIComponent(resolvedSlug),
                requestOptions,
              );
        ...
        fetchBlogComments(config, state.post.id, requestOptions)
          .then(function (result) {
            ...
            setCommentsState({
              ...
              submitEnabled:
                !!config.commentSubmitEnabled && result.submitEnabled,
              total: result.total,
        ...
        var raf = requestAnimationFrame(function () {
          ...
          codeBlocks.forEach(function (block) {
            ...
            copyButton.addEventListener("click", function () {
              ...
              var copyText = copyButton.querySelector(
              ...
              copyCodeWithFallback(codeText).then(function (didCopy) {
                ...
                copyButton._hdcCopyResetTimeout = window.setTimeout(
                  function () {
      ...
      function () {
        ...
        var ld = {
          ...
          image: state.post.featuredImageUrl
            ? [
                new URL(state.post.featuredImageUrl, window.location.origin)
                  .href,
              ]
            : undefined,
    ...
    const articlePreview =
      (post.excerpt || "").trim() || (post.seoDescription || "").trim() || "";
    ...
    const resolvedRelatedPosts = relatedPosts.reduce(function (
      ...
      if (isMirroredPost) {
        resolved.push({
          href:
            config.blogIndexUrl.replace(/\/+$/, "") + "/" + related.slug + "/",
          ...
          if (result.comment && "approved" === result.moderationStatus) {
            setCommentsState(function (currentState) {
              const nextComments = dedupeComments(
                currentState.comments.concat([result.comment]),
              );
              return Object.assign({}, currentState, {
    ...
    return h(
      "div",
      {
        ...
        className:
          "hdc-blog-post__root" +
          (prefersReducedMotion ? "" : " hdc-blog-post__root--entering"),
      },
      config.showProgress
        ? h(
            "div",
      ...
      h(
        ...
        h(
          ...
          h(
            ...
            articlePreview
              ? h("p", { className: "hdc-blog-post__lede" }, articlePreview)
              : null,
            h(
              "p",
              ...
              post.readingTime
                ? h(
                    ...
                    h(
                      "span",
                      {
                        className: "hdc-blog-post__meta-icon-glyph",
                        "aria-hidden": "true",
                      },
                      renderLucideIcon(h, "clock", {
                        className: "hdc-blog-post__meta-icon-svg",
                        size: 12,
                      }),
                    ),
                  ...
                  authorLink
                    ? h(
                        ...
                        h(
                          "a",
                          {
                            className: "hdc-blog-post__detail-link",
                            href: authorLink,
                            rel: "noopener noreferrer",
                            target: "_blank",
                          },
                  ...
                  postHasUpdatedDate && post.modifiedDate
                    ? h(
                        ...
                        h(
                          ...
                          formatLongDateLabel(post.modifiedDate),
                        ),
                      )
                    : null,
                  categoriesMeta || null,
          ...
          h(
              ...
              h(
                ...
                h(
                  ...
                  h(
                    ...
                    h(
                      "p",
                      { className: "hdc-blog-post__eyebrow" },
                      "Discussion",
                    ),
                    h(
                      "div",
                      ...
                      h(
                        "h2",
                        {
                          className: "hdc-blog-post__details-title",
                      ...
                      shouldShowCommentCount
                        ? h(
                            "span",
                            { className: "hdc-blog-post__count-badge" },
                            publishedCommentCount === 1
                              ? "1 published comment"
                              : String(publishedCommentCount) +
                                  " published comments",
                          )
                        : null,
                    ),
                  ...
                  discussionHref
                    ? h(
                        ...
                        h(
                          "span",
                          {
                            className: "hdc-blog-post__share-link-icon",
                            "aria-hidden": "true",
                          },
                          renderLucideIcon(h, "message-square", { size: 16 }),
                        ),
                        h(
                          "span",
                          null,
                ...
                hasWordPressThread && commentsState.isLoading
                  ? h(
                      ...
                      renderLucideIcon(h, "loader-2", {
                        className: "hdc-blog-post__spinner",
                        size: 16,
                      }),
                      h(
                        "span",
                  ...
                  ? h(
                      ...
                      commentTree.map(function (comment) {
                        return renderCommentThreadItem({
                          ...
                          isReplyDisabled:
                            isCommentSubmitting || isCommentVerificationPending,
                  ...
                  ? h(
                      ...
                      h(
                        ...
                        h(
                          ...
                          h(
                            "p",
                            { className: "hdc-blog-post__share-description" },
                            commentReplyTarget
                              ? "Replying to " +
                      ...
                      commentReplyTarget
                        ? h(
                            "p",
                            { className: "hdc-blog-post__reply-indicator" },
                      ...
                      commentSubmitMessage
                        ? h(
                            "div",
                            { className: "hdc-blog-post__submit-message" },
                            commentSubmitMessage,
                          )
                        : null,
                      commentSubmitError
                      ...
                      h(
                        "form",
                        {
                          className: "hdc-blog-post__comment-form-grid",
                          onSubmit: handleCommentSubmit,
                        },
                        h("input", {
                          "aria-hidden": "true",
                          autoComplete: "off",
                          className: "hdc-blog-post__sr-only",
                          name: "company",
                        ...
                        h(
                          ...
                          h(
                            "label",
                            {
                              className: "hdc-blog-post__field",
                              htmlFor: "comment-author-name",
                            },
                            h(
                              "span",
                              { className: "hdc-blog-post__field-label" },
                              "Name",
                            ),
                            h("input", {
                              "aria-invalid": commentFieldErrors.authorName
                                ? "true"
                                : undefined,
                              className: "hdc-blog-post__input",
                              disabled:
                                isCommentSubmitting ||
                                isCommentVerificationPending,
                          ...
                          h(
                            ...
                            h("input", {
                              "aria-invalid": commentFieldErrors.authorEmail
                                ? "true"
                                : undefined,
                              className: "hdc-blog-post__input",
                              disabled:
                        ...
                        h(
                          ...
                          h("input", {
                            "aria-invalid": commentFieldErrors.authorUrl
                              ? "true"
                              : undefined,
                            className: "hdc-blog-post__input",
                            disabled:
                        ...
                        h(
                          ...
                          h("textarea", {
                            "aria-invalid": commentFieldErrors.content
                              ? "true"
                              : undefined,
                            className: "hdc-blog-post__textarea",
                            disabled:
                              isCommentSubmitting ||
                              isCommentVerificationPending,
                            id: "comment-content",
                        ...
                        h(
                          ...
                          h(TurnstileWidget, {
                            ...
                            ariaDescribedBy: commentTurnstileError
                        ...
                        h(
                          ...
                          h(
                            "button",
                            {
                              className: "hdc-blog-post__submit-button",
                              disabled:
                                isCommentSubmitting ||
                                isCommentVerificationPending,
                              type: "submit",
                ...
                h(
                  ...
                  h(
                    ...
                    h(
                      "span",
                      {
                        className: "hdc-blog-post__share-copy-icon",
                        "aria-hidden": "true",
                      },
                      renderLucideIcon(h, "share-2", { size: 14 }),
                    ),
                    h(
                      "span",
                      { className: "hdc-blog-post__share-copy-text" },
                      "Copy article link",
                    ),
                  ),
                  h(
                    ...
                    h(
                      ...
                      renderLucideIcon(h, "linkedin", { size: 14 }),
                    ),
                  ...
                  h(
                    ...
                    h(
                      ...
                      renderLucideIcon(h, "mail", { size: 14 }),
                    ),
            ...
            resolvedRelatedPosts.length
              ? h(
                  ...
                  h(
                    "h3",
                    { className: "hdc-blog-post__related-title" },
                    "Related Posts",
                  ),
                  h(
                    "div",
                    ...
                    resolvedRelatedPosts.map(function (relatedPost) {
                      return h(
                        ...
                        h(
                          "h4",
                          { className: "hdc-blog-post__related-card-title" },
                          relatedPost.post.title,
                        ),
                        h(
                          "p",
                          { className: "hdc-blog-post__related-card-excerpt" },
            ...
            h(
              ...
              h(
                ...
                renderLucideIcon(h, "chevron-up", {
                  className: "hdc-blog-post__scroll-top-icon-svg",
                  size: 18,
                }),
              ),
            ),

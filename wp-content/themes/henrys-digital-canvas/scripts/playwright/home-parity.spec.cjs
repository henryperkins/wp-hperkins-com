...
const SECTIONS = [
	{
		name: 'shell',
		sourceSelector: '.hdc-home-page__shell',
		targetSelector: '.hdc-home-page__shell',
	},
	{
		name: 'hero',
		sourceSelector: '.hdc-home-page__hero',
		targetSelector: '.hdc-home-page__hero',
	},
	{
		name: 'selected-work',
	...
	},
	{
		...
		targetSelector:
			'[data-hdc-home-resume-snapshot].hdc-home-page__section',
	},
	{
		name: 'recent-writing',
	...
	{
		name: 'contact-cta',
		sourceSelector: '.hdc-home-page__cta-card',
		targetSelector: '.hdc-home-page__cta-card',
	},
];
...
function normalizeClassList( classList ) {
	return classList
		.filter(
			( className ) => ! WORDPRESS_WRAPPER_CLASS_PATTERN.test( className )
		)
		.filter(
			( className ) => ! MIGRATION_MARKER_CLASS_PATTERN.test( className )
		)
		.filter(
			( className ) =>
				className !== 'hdc-reveal' &&
				className !== 'hdc-reveal--fade-in'
		)
		.sort();
}
...
test.describe( 'home page structural parity', () => {
	...
	for ( const section of SECTIONS ) {
		test( `section "${ section.name }" matches source DOM + computed styles`, async ( {
			browser,
		} ) => {
			...
			await sourcePage.goto( `${ SOURCE_BASE_URL }/`, {
				waitUntil: 'networkidle',
			} );
			await targetPage.goto( `${ TARGET_BASE_URL }/`, {
				waitUntil: 'networkidle',
			} );

			const sourceInfo = await describeSection(
				sourcePage,
				section.sourceSelector
			);
			const targetInfo = await describeSection(
				targetPage,
			...
			expect(
				targetInfo.exists,
				`target ${ section.targetSelector } must exist`
			).toBe( true );
			expect(
				sourceInfo.exists,
				`source ${ section.sourceSelector } must exist`
			).toBe( true );
			expect( targetInfo.tagName ).toBe( sourceInfo.tagName );
			expect( normalizeClassList( targetInfo.classList ) ).toEqual(
				normalizeClassList( sourceInfo.classList )
			);

...
const BLOG_MISSING_SLUG =
	process.env.BLOG_MISSING_SLUG || 'this-slug-should-not-exist';
const WORK_DETAIL_REPO =
	process.env.WORK_DETAIL_REPO || 'henry-s-digital-canvas';
...
function buildExpectedBlogTitle( post ) {
	const baseTitle =
		ensureString( post && post.seoTitle ) ||
		ensureString( post && post.title ) ||
		'Post Not Found';
	return /henry perkins/i.test( baseTitle )
		? baseTitle
		: `${ baseTitle } — Henry Perkins`;
}
...
async function resolveBlogFixtures( request ) {
	const listPayload = await fetchJson(
		request,
		`/wp-json/henrys-digital-canvas/v1/blog?limit=${ BLOG_LIST_LIMIT }`
	);
	const posts = Array.isArray( listPayload && listPayload.posts )
		? listPayload.posts.filter(
				( post ) => post && ensureString( post.slug )
		  )
		: [];

	if ( ! posts.length ) {
		throw new Error(
			'Could not resolve a published blog post for browser smoke checks.'
		);
	}
	...
	async function getDetail( slug ) {
		...
		const detail = await fetchJson(
			request,
			`/wp-json/henrys-digital-canvas/v1/blog/${ slug }`
		);
...
test.describe( 'Henrys Digital Canvas browser smoke', () => {
	...
	test( 'contact validation and success states', async ( { page } ) => {
		...
		const verificationShell = page.locator(
			'.hdc-contact-form__verification-shell'
		);
		await expect( verificationShell ).toHaveCount( 1, { timeout: 10000 } );
		if ( await submitButton.isDisabled() ) {
			await expect(
				page.locator( '#contact-verification-error' )
			).toContainText( /verification/i );
			...
			await expect
				.poll(
					async () =>
						page
							.locator( '.hdc-contact-form__hint--error' )
							.count(),
					{ timeout: 5000 }
				)
				.toBeGreaterThan( 0 );
			...
			await page.fill(
				'.hdc-contact-form input[name="name"]',
				'Phase 6 Bot'
			);
			await page.fill(
				'.hdc-contact-form input[name="email"]',
				'phase6-bot@example.com'
			);
			await page.fill(
				'.hdc-contact-form textarea[name="message"]',
				'Phase 6 browser smoke validation message.'
			);
			await submitButton.click();
			await expect(
				page.locator( '#contact-verification-error' )
			).toContainText( /verification/i, { timeout: 10000 } );
		}
	} );

	test( 'blog progress and scroll-to-top behavior', async ( { page } ) => {
		await page.goto( `/blog/${ blogFixtures.blogSlug }/`, {
			waitUntil: 'networkidle',
		} );

		const progressFill = page
			.locator( '.hdc-blog-post__progress-fill' )
			.first();
	...
	test( 'blog detail missing-post route renders the enhanced state card', async ( {
		page,
	} ) => {
		await page.goto( `/blog/${ BLOG_MISSING_SLUG }/`, {
			waitUntil: 'networkidle',
		} );

		await expect(
			page.locator( '.hdc-blog-post__state-card' )
		).toHaveCount( 1, { timeout: 10000 } );
		await expect(
			page.getByRole( 'heading', { level: 2, name: 'Post not found' } )
		).toBeVisible();
		await expect(
	...
	test( 'blog detail enhances HTML code blocks with a toolbar and copy affordance', async ( {
		page,
	} ) => {
		test.skip(
			! blogFixtures.codeSlug,
			'No published blog post with an HTML code block is available.'
		);

		await page.goto( `/blog/${ blogFixtures.codeSlug }/`, {
			waitUntil: 'networkidle',
		} );
		...
		await expect
			.poll(
				async () =>
					page.locator( '.hdc-blog-post__code-toolbar' ).count(),
				{ timeout: 10000 }
			)
			.toBeGreaterThan( 0 );
		await expect(
			page.locator( '.hdc-blog-post__code-copy' ).first()
		).toHaveCount( 1 );
		await expect(
			page.locator( '.hdc-blog-post__code-language' ).first()
		).not.toHaveText( '' );
	} );

	test( 'blog detail renders comments, share, related posts, and canonical route metadata', async ( {
		page,
	} ) => {
		const htmlResponse = await page.request.get(
			`/blog/${ blogFixtures.blogSlug }/`
		);
		...
		await expect(
			page.locator( '.hdc-blog-post__comments-card' )
		).toHaveCount( 1 );
		await expect(
			page.getByRole( 'button', { name: 'Copy article link' } )
		).toBeVisible();
		await expect(
		...
		await expect(
			page.getByRole( 'link', {
				name: /Comment on WordPress|Open original post/,
			} )
		).toBeVisible();
		await expect( page.locator( '.hdc-blog-post__related' ) ).toHaveCount(
			blogFixtures.posts.length > 1 ? 1 : 0
		);

		expect( html ).toContain(
			`<link rel="canonical" href="https://hperkins.com/blog/${ blogFixtures.blogSlug }" />`
		);
		expect( html ).toContain(
			'<meta property="og:type" content="article" />'
		);
		expect( html ).toContain(
			`<meta property="og:url" content="https://hperkins.com/blog/${ blogFixtures.blogSlug }" />`
		);
		expect( html ).toContain(
			`<meta name="twitter:title" content="${ escapeHtmlAttribute(
				buildExpectedBlogTitle( blogFixtures.blogDetail )
			) }" />`
		);
	} );

	test( 'blog and home media surfaces follow blog media contract', async ( {
		page,
	} ) => {
		...
		const posts = Array.isArray( listPayload && listPayload.posts )
			...
			: [];
		const featuredPost =
			posts.find( ( post ) => post && post.featured ) ||
			posts[ 0 ] ||
			null;
		...
		const blogFeaturedImage = page.locator(
			'.hdc-blog-index__featured-image'
		);
		const blogRowThumbs = page.locator( '.hdc-blog-index__card-thumb' );

		if ( featuredPostHasImage ) {
			await expect( blogFeaturedImage ).toHaveCount( 1, {
				timeout: 10000,
			} );
		} else {
		...
		if ( hasAnyListThumbnail ) {
			await expect
				.poll( async () => blogRowThumbs.count(), { timeout: 10000 } )
				.toBeGreaterThan( 0 );
		} else {
		...
		const detailResponse = await page.request.get(
			`/wp-json/henrys-digital-canvas/v1/blog/${ blogFixtures.blogSlug }`
		);
		...
		if ( detailHasImage ) {
			await expect( detailHeroImage ).toHaveCount( 1, {
				timeout: 10000,
			} );
		} else {
		...
		if ( hasAnyListThumbnail ) {
			await expect
				.poll( async () => homePostThumbs.count(), { timeout: 10000 } )
				.toBeGreaterThan( 0 );
		} else {
	...
	test( 'not-found recovery, command palette, and mobile menu', async ( {
		page,
		browser,
	} ) => {
		await page.goto( '/about/', { waitUntil: 'networkidle' } );
		await page.goto( '/route-should-not-exist/', {
			waitUntil: 'networkidle',
		} );
		...
		await expect(
			page
				.locator( '.hdc-site-shell__command-item.is-highlighted' )
				.first()
		).toContainText( 'About' );
